...
class ModelClassify(ALBEF):
    def inference(self, image, text_input=None, use_embeds=False):
        ...
        logit = self.cls_head(output['fusion_output'][:, 0, :])
...
def evaluate(model, ref_model, data_loader, tokenizer, device, config):
    ...
    if args.attack == 'MI':
        image_attacker = ImageAttack_MI(config['epsilon'] / 255., preprocess=images_normalize, bounding=(0, 1), cls=args.cls)
    elif args.attack == 'PGD':
        image_attacker = ImageAttacker(config['epsilon'] / 255., preprocess=images_normalize, bounding=(0, 1), cls=args.cls)
    elif args.attack == 'SI':
        image_attacker = ImageAttack_SI(config['epsilon'] / 255., preprocess=images_normalize, bounding=(0, 1), cls=args.cls, repeat=5)
    else:
        raise ('Not support attack method')
    text_attacker = BertAttackFusion(ref_model, tokenizer, cls=False)
