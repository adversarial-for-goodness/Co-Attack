...
def retrieval_eval(model, ref_model, data_loader, tokenizer, device, config):
    ...
    for images, texts, texts_ids in data_loader:
        ...
        if args.adv != 0:
            images, texts = multi_attacker.run(images, texts, adv=args.adv, num_iters=config['num_iters'])
...
def main(args, config):
    ...
    ref_model = torch.nn.parallel.DataParallel(ref_model, device_ids=args.gpu)
    model_without_ddp = model.module
    ref_model_without_ddp = ref_model.module

    print("Start eval")
    start_time = time.time()


    score_i2t, score_t2i = retrieval_eval(model_without_ddp, ref_model_without_ddp, test_loader, tokenizer, device, config)
    ...
    log_stats = {**{f'test_{k}': v for k, v in result.items()},
                 'eval type': args.adv, 'eps': config['epsilon'], 'attack': args.attack}
