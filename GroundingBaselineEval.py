...
class ModelClassify(ALBEF):
    def inference(self, image, text_input=None, use_embeds=False):
        output = super(ModelClassify, self).inference(image, text_input, use_embeds)
        logit = self.itm_head(output['fusion_output'][:, 0, :])
        output['predict'] = logit
        return output
...
def val(model, ref_model, data_loader, tokenizer, device, block_num):
    ...
    if args.attack == 'MI':
        image_attacker = ImageAttack_MI(config['epsilon'] / 255., preprocess=images_normalize, bounding=(0, 1),
                                        cls=args.cls)
    elif args.attack == 'PGD':
        image_attacker = ImageAttacker(config['epsilon'] / 255., preprocess=images_normalize, bounding=(0, 1),
                                       cls=args.cls)
    elif args.attack == 'SI':
        image_attacker = ImageAttack_SI(config['epsilon'] / 255., preprocess=images_normalize, bounding=(0, 1),
                                        cls=args.cls, repeat=5)
    else:
        raise ('Not support attack method')
    text_attacker = BertAttackFusion(ref_model, tokenizer, cls=args.cls)
    multi_attacker = MultiModalAttackerClassify(model, image_attacker, text_attacker, tokenizer, cls=args.cls)
    ...
    for images, text, ref_ids in metric_logger.log_every(data_loader, print_freq, header):
        ...
        if args.adv != 0:
            ...
            images, text = multi_attacker.run(images,text,targets,adv=args.adv, num_iters=config['num_iters'])
...
def main(args, config):
    ...
    model = ModelClassify(config = config, text_encoder=args.text_encoder, tokenizer=tokenizer)
    ...
    log_stats = {**{f'{k}': v for k, v in grounding_acc.items()},
                 'eval type': args.adv, 'attack': args.attack}
